...
class XPS:
        ...
        def wrapper(*args, **kw):
            try:
                ...
                if XPS.__usedSockets[sid] == 0:
                    raise XPSException(
                        'invalid socket at function %s' % fcn.__name__)
            except IndexError:
                raise XPSException(
                    'no socket specified for fucntion %s' % fcn.__name__)
            return fcn(*args, **kw)
    ...
    def TCP_ConnectToServer(self, IP, port, timeOut):
        ...
        XPS.__nbSockets += 1
        try:
            XPS.__sockets[socketId] = socket.socket(
    ...
    def ControllerStatusStringGet(self, socketId, ControllerStatusCode):
        command = 'ControllerStatusStringGet(%s, char *)' % str(
    ...
    def PositionerAnalogTrackingVelocityParametersGet(self, socketId, PositionerName):
        outputs = XPSOutputs('char', 'double', 'double',
    ...
    def PositionerCorrectorPIDFFAccelerationGet(self, socketId, PositionerName):
        outputs = XPSOutputs('bool', 'double', 'double', 'double', 'double',
    ...
    def PositionerCorrectorPIDDualFFVoltageGet(self, socketId, PositionerName):
        outputs = XPSOutputs('bool', 'double', 'double', 'double', 'double', 'double',
    ...
    def PositionerDACOffsetSet(self, socketId, PositionerName, DACOffset1, DACOffset2):
        command = 'PositionerDACOffsetSet(' + PositionerName + \
            ',' + str(DACOffset1) + ',' + str(DACOffset2) + ')'
        return self.Send(socketId, command)
    ...
    def PositionerDACOffsetDualSet(self, socketId, PositionerName, PrimaryDACOffset1, PrimaryDACOffset2, SecondaryDACOffset1, SecondaryDACOffset2):
        command = 'PositionerDACOffsetDualSet(' + PositionerName + ',' + str(PrimaryDACOffset1) + ',' + str(
            PrimaryDACOffset2) + ',' + str(SecondaryDACOffset1) + ',' + str(SecondaryDACOffset2) + ')'
        return self.Send(socketId, command)
    ...
    def MultipleAxesPTVerification(self, socketId, GroupName, TrajectoryFileName):
        command = 'MultipleAxesPTVerification(' + \
            GroupName + ',' + TrajectoryFileName + ')'
        return self.Send(socketId, command)
    ...
    def MultipleAxesPTExecution(self, socketId, GroupName, TrajectoryFileName, ExecutionNumber):
        command = 'MultipleAxesPTExecution(' + GroupName + ',' + \
            TrajectoryFileName + ',' + str(ExecutionNumber) + ')'
        return self.Send(socketId, command)
