...
class NewPortXPS(Menu):
    def __init__(self, root):
        ...
        s.configure("Sty1.TButton", font=("Helvetica", self.fontsize))
        s.configure("Sty1.TEntry", font=("Helvetica", self.fontsize))
        root.option_add("*TCombobox*Listbox.font", "Helvetica 20")

        s.configure("Sty1.TCombobox", font=("Helvetica", self.fontsize))
        for idx, (key, value) in enumerate(self.menu_dict.items()):
            # value[0][:-3], i.e., 'server' is used as the StringVar name.
            setattr(self, value[0][:-3], StringVar())
            setattr(
                self,
                f"{value[0][:-3]}_combobox",
                ttk.Combobox(
                    frame1,
                    textvariable=getattr(self, value[0][:-3]),
            ...
            ttk.Button(
                frame1,
                text=f"{key}",
                command=partial(self.start_window, __file__, f"{key}"),
                style="Sty1.TButton",
            ).grid(column=1, row=idx, columnspan=1, sticky=[W, E])
            getattr(self, f"{value[0][:-3]}_combobox")["value"] = value[1]
            ttk.Button(
                frame1,
                text="X",
                command=partial(self.terminate, f"{key}"),
                style="Sty1.TButton",
                width=5,
            ).grid(column=2, row=idx, columnspan=1, sticky=[W])
